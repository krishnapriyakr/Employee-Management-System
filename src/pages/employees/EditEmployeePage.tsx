...
import type { Employee, UpdateEmployeeData } from '../../types';
import { editEmployee, fetchEmployeeById } from '../../api/employeeApi';
...
const EditEmployeePage: React.FC = () => {
      ...
      if (response.success) {
        ...
        setFormData({
          personalInfo: {
            ...
            dateOfBirth: employee.personalInfo.dateOfBirth.split('T')[0], // Format date for input
          ...
          employmentInfo: {
            ...
            joiningDate: employee.employmentInfo.joiningDate.split('T')[0], // Format date for input
  ...
  const handleInputChange = (section: keyof UpdateEmployeeData, field: string, value: any) => {
    setFormData(prev => ({
      ...prev,
      [section]: {
        ...prev[section],
  ...
  // Validation functions (same as AddEmployeePage)
  ...
  const handleSubmit = async () => {
    try {
      setIsLoading(true);
      const response = await editEmployee(id!, formData as any);
  ...
  const hasChanges = () => {
    ...
    return (
      formData.personalInfo?.firstName !== originalData.personalInfo.firstName ||
      formData.personalInfo?.lastName !== originalData.personalInfo.lastName ||
      formData.personalInfo?.email !== originalData.personalInfo.email ||
      formData.personalInfo?.phone !== originalData.personalInfo.phone ||
      formData.personalInfo?.dateOfBirth !== originalData.personalInfo.dateOfBirth.split('T')[0] ||
      formData.personalInfo?.gender !== originalData.personalInfo.gender ||
      formData.employmentInfo?.department !== originalData.employmentInfo.department ||
      formData.employmentInfo?.position !== originalData.employmentInfo.position ||
      formData.employmentInfo?.salary !== originalData.employmentInfo.salary ||
      formData.employmentInfo?.joiningDate !== originalData.employmentInfo.joiningDate.split('T')[0] ||
      formData.employmentInfo?.status !== originalData.employmentInfo.status ||
      ...
      formData.emergencyContact?.name !== originalData.emergencyContact.name ||
      formData.emergencyContact?.relationship !== originalData.emergencyContact.relationship ||
      formData.emergencyContact?.phone !== originalData.emergencyContact.phone
    );
  };

  // Render step progress (same as AddEmployeePage)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input
            type="number"
            value={formData.employmentInfo?.salary || 0}
  ...
  const renderReview = () => (
    <div className="space-y-6">
      <h3 className="text-lg font-semibold text-gray-800">Review Changes</h3>
        ...
        {hasChanges() && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <h3 className="text-sm font-medium text-yellow-800">Changes Detected</h3>
                <p className="text-sm text-yellow-700 mt-1">
                  You have made changes to this employee's information. Click "Update Employee" to save your changes.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex space-x-3">
              {currentStep !== 'review' ? (
                ...
                <button
                  ...
                  className={`bg-green-500 text-white px-6 py-2 rounded-lg font-medium transition-colors duration-200 ${
